...
import org.highway.service.ServiceTagNames;
 ...
 * @author David Attias
 */
public class TransactionInterceptorJavadoc implements ServiceInterceptor,
	TransactionOptions
{
	/**
	 * Method invoke
	...
	{
		String option = getTransactionOption(request.getMethod());
		...
		if (option.equals(REQUIRED))
		{
			return invokeRequired(request);
		}

		if (option.equals(REQUIRES_NEW))
		{
			return invokeRequiresNew(request);
		}

		if (option.equals(SUPPORTS))
		{
			return invokeSupports(request);
		}

		if (option.equals(NOT_SUPPORTED))
		{
	...
	private void setTransactionTimeout(ServiceRequest request)
	{
		String timeoutString =
			JavadocHome.getMethodTag(
				request.getMethod(), ServiceTagNames.TRANSACTION_TIMEOUT);

		// default timeout = 5 seconds
		int timeout =
			(timeoutString == null) ? 5000 : Integer.parseInt(timeoutString);
	 ...
	 * @return String
	 */
	private String getTransactionOption(Method method)
	{
		return JavadocHome.getMethodTag(
			method, ServiceTagNames.TRANSACTION_OPTION);
	}
}
